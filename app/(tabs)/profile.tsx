...
export default function ProfileScreen() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: t.bg }} contentContainerStyle={s.content}>
      ...
      <View style={[s.hero, { backgroundColor: palette.blue }]}>
        <TouchableOpacity style={s.avatarWrap} onPress={pickAvatar} activeOpacity={0.85}>
          ...
          <View style={s.cameraBtn}>
            <Ionicons name="camera" size={14} color={palette.blue} />
          </View>
        </TouchableOpacity>
      ...
      <SectionCard t={t}>
        <SectionHeader
          icon="person-outline" title="Личные данные"
          action={!editing ? (
            <TouchableOpacity
              style={[s.editBtn, { backgroundColor: `${palette.blue}15` }]}
              onPress={() => setEditing(true)}
            >
              <Ionicons name="pencil" size={14} color={palette.blue} />
        ...
        {editing ? (
          ...
          <>
            <InfoRow label="Фамилия" value={user?.lastName ?? '—'} t={t} />
            <InfoRow label="Имя" value={user?.firstName ?? '—'} t={t} />
            <InfoRow label="Email" value={user?.email ?? '—'} t={t} />
            <InfoRow label="Телефон" value={user?.phone ?? '—'} t={t} last />
          </>
        )}
      </SectionCard>
          ...
          <View style={[s.pwdBlock, { borderTopColor: t.border }]}>
            {([
              ...
              <View key={key} style={s.formField}>
                ...
                <View style={[
                  s.inputWrap,
                  { backgroundColor: t.inputBg, borderColor: pwdErrors[key] ? palette.red : t.border },
                ]}>
            ...
            <TouchableOpacity
              style={[s.saveBtn, { backgroundColor: palette.purple }, savingPwd && s.disabled]}
...
const s = StyleSheet.create({
  ...
  hero: { padding: 28, alignItems: 'center', gap: 8, paddingBottom: 32 },
  avatarWrap: { position: 'relative', marginBottom: 4 },
  avatarImg: { width: 90, height: 90, borderRadius: 30 },
  avatarPlaceholder: {
  ...
  heroLogin: { fontSize: 14, color: 'rgba(255,255,255,0.7)' },
  policiesRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 6, justifyContent: 'center' },
  policyPill: { backgroundColor: 'rgba(255,255,255,0.2)', paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  ...
  pwdBlock: { borderTopWidth: 1, paddingTop: 12, paddingBottom: 8 },

  logoutBtn: {
    flexDirection: 'row', alignItems: 'center', gap: 12,
    margin: 16, marginTop: 12, borderRadius: 20, padding: 16,
    borderWidth: 1.5,
